...
class Vocab:
    ...
    def __init__(self, vocab: Iterable[str]) -> None:
        self.token2idx = {
            Vocab.PAD: 0,
            Vocab.UNK: 1,
            **{token: i for i, token in enumerate(vocab, start=2)},
        }
    ...
    #encode word into word id
    def encode(self, tokens: List[str]) -> List[int]:
        return [self.token_to_id(token) for token in tokens]

    #encode word into word id and pad the whole sequence to "to_len" length
    def encode_batch(
        self, batch_tokens: List[List[str]], to_len: int = None
    ) -> List[List[int]]:
        batch_ids = [self.encode(tokens) for tokens in batch_tokens]
        #pad to maximum sequence length or 'to_len'
...
def pad_to_len(seqs: List[List[int]], to_len: int, padding: int) -> List[List[int]]:
    # pad at the front so that h_last is more "fresh", closer to actual data
    paddeds = [
        [padding] * max(0, to_len - len(seq)) + seq[:to_len] for seq in seqs
        ]
    return paddeds
